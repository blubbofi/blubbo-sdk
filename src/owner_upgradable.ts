...
export class OwnerUpgradable {
  static blubbo_user_version_bits = 14;
  static blubbo_user_variant_bits = 4;
  ...
  static pack_data(
    ...
    return beginCell()
      ...
      .storeUint(blubbo_user_version, OwnerUpgradable.blubbo_user_version_bits)
      .storeUint(variant, OwnerUpgradable.blubbo_user_variant_bits)
      .endCell();
  }
  ...
  //     .begin_parse();
  // }
  static calculate_address_internal(state_init: Cell): Slice {
    return beginCell()
      .storeUint(4, 3)
      .storeInt(0, 8)
   ...
   *
   * @param user_wallet_address The address of the owner (user wallet)
   * @param blubbo_master_address The address of the blubbo master
   * @param owner_upgradable_code The code of the Owner Upgradable contract
   * @returns The address of the blubbo user contract belonging to the owner
   */
  public static calculate_owner_upgradable_address(
    user_wallet_address: Address,
    blubbo_master_address: Address,
    owner_upgradable_code: Cell,
  ): Address {
    return OwnerUpgradable.calculate_address_internal(
      OwnerUpgradable.calculate_state_init(
        user_wallet_address,
        blubbo_master_address,
