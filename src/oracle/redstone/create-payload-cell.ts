...
function storeMetadata(metadataHex: string, builder: Builder) {
    const maxUnsignedDataLength = Math.floor(
        builder.availableBits / 8 -
            (consts.REDSTONE_MARKER_BS + consts.DATA_PACKAGES_COUNT_BS + consts.UNSIGNED_METADATA_BYTE_SIZE_BS),
    );
...
function storeSignatureAndData(dataPackageHex: string, builder: Builder) {
    ...
    const signature = dataPackageHex.substring(dataPackageHex.length - 2 * consts.SIGNATURE_BS, dataPackageHex.length);
