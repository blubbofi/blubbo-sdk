  ...
  static async getLatestPricesFromOracle() {
    for (let i = 0; i < RedstoneOracleConstants.ORACLE_RETRY_COUNT; i++) {
      try {
        ...
        console.error(
          `Error fetching prices from oracle: ${err}. Retry count: ${i}`,
        );
        await new Promise((resolve) =>
          setTimeout(resolve, RedstoneOracleConstants.ORACLE_RETRY_DELAY_MS),
        );
      }
    }
  ...
  private static async getLatestPricesFromOracleInternal() {
    ...
    const signedDataPackagesResponse = await requestDataPackages({
      ...
      maxTimestampDeviationMS:
        RedstoneOracleConstants.MAX_TIMESTAMP_DEVIATION_MS,
    });
    ...
    for (const [dataFeedId, dataPackages] of Object.entries(
      ...
      for (const signedDataPackage of dataPackages) {
        if (
          now - signedDataPackage.dataPackage.timestampMilliseconds >
          RedstoneOracleConstants.PRICE_TTL_MS
        ) {
          throw new Error(`Stale price data package for ${dataFeedId}`);
        }
        const signerAddress = signedDataPackage.recoverSignerAddress();
        if (
          !RedstoneOracleConstants.AUTHORIZED_SIGNER_ADDRESSES_BIGINT_SET.has(
        ...
        if (dataServiceBySigner !== RedstoneOracleConstants.DATA_SERVICE_ID) {
          throw new Error(
            `Invalid data service id for signer ${signerAddress}`,
          );
        }
